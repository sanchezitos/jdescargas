    ...
    if (name.length > 0) {
        ...
        if (ext === '.png' || ext === '.jpg' || ext === '.jpeg' || ext === '.gif') {
            ...
            const newNotification = new Notification({
                action: 'new_movie',
                type: 'movie',
                movie: movieSaved.title,
                movie_id: movieSaved.id,
                user_id: movieSaved.user,
                movies: 'ha añadido la pelicula'
            });
...
router.post('/movies/add_report', isAuthenticated, async(req, res) => {
    ...
    const newNotification = new Notification({
        ...
        user_id: user_id,
        user: user_name,
        report: 'ha añadido un nuevo reporte a la pelicula'
    });
    await newNotification.save();
...
router.put('/movies/add_like/:id', isAuthenticated, async(req, res) => {
    ...
    movie = await Movie.findByIdAndUpdate(req.params.id, { $push: { likes: id_user } }, { strict: false }, );
...
router.put('/movies/add_mylist/:id', isAuthenticated, async(req, res) => {
    const { id_user } = req.body;
    movie = await Movie.findByIdAndUpdate(req.params.id, { $push: { addtomylist: id_user } }, { strict: false }, );
    const newNotification = new Notification({
        action: 'new_add_list',
        type: 'list',
        movie: movie.title,
        ...
        user_id: user.id,
        user: user.name,
        list: 'ha añadido a su lista la pelicula'
    });
    await newNotification.save();
    res.redirect('/movies/' + movie.title);
